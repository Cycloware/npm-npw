...
`);
            ...
            const packageResult = yield getPackageInfo_1.getPackageInfo(absolutePackagePath);
...
`);
  ...
  ${errorInstallPaths.map(p => p.errorMessage).join('  \n')}`));
                        ...
                        return tslib_1.__awaiter(this, void 0, void 0, function* () {
                            ...
                            if (validateSourcesExist) {
                                function sourceValidator() {
                                    return tslib_1.__awaiter(this, void 0, void 0, function* () {
                                        try {
                                            const statsSource = yield getStatInfo_1.getStatInfo.Async(absoluteSourcePathClean, false);
                                            if (statsSource.result === 'stat-returned') {
                                                if (statsSource.isDirectory) {
                                                    if (moduleNameVerification) {
                                                        ...
                                                        const resPackageInfo = yield getPackageInfo_1.getPackageInfo(sourcePackageInfoPath);
    ...
    ${'*** names only differ by case ***'.red}`;
                    ...
                    const res = uninstall ? [] : yield Promise.all(packagesToLink.map(val => linkModuleAsync(val)));
