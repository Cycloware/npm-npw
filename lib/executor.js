...
function executor(commandTextIn, argsIn, argsAsIs) {
  ...
  buildCommandOptions(['--list-globals', '--list-global', '-list-globals', '-list-global'],
  ...
  try {
    ...
    if (lineEnding !== 'none') {
      linesPromise = linesPromise.then(() => {
        spawnCaller.lines(lineEnding, verbose);
      })
    }
